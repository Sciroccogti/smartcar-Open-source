...
mcg_cfg_t mcg_cfg[PLL_MAX] =
{
    ...
    {100,   7,      16      },                          //PLL100
    {110,   4 ,     6       },                          //PLL110
    {120,   4 ,     8       },                          //PLL120  // 50*(8+16)/(4+1)/2=120MHz
    {130,   4 ,     10      },                          //PLL130
    {140,   4 ,     12      },                          //PLL140
    {150,   4 ,     14      },                          //PLL150
    {160,   4 ,     16      },                          //PLL160
...
{
    uint16 clk = mcg_cfg[pll].clk;  // clk = 120 (i.e. 120MHz for PLL120)
    if( (pll > PLLUSR) && (pll < PLL_MAX) )
    {
        ...
        else if(clk <= 2 * MAX_BUS_CLK)   mcg_div.bus_div = 1;          // bus  = MCG/2  // clk=120, // MAX_BUS_CLK=60
        ...
        else if(clk <= 5 * MAX_FLEXBUS_CLK)   mcg_div.flex_div = 4;     // flex bus  = MCG/5   // MAX_FLEXBUS_CLK=25MHz
        ...
        else if(clk <= 5 * MAX_FLASH_CLK)   mcg_div.flash_div = 4;      // flash  = MCG/5  // MAX_FLASH_CLK = 25
