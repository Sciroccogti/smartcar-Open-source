...
{
    uint8  br,pbr;
    uint32 clk = bus_clk_khz*1000/baud;  // bus_clk_khz = 60,000 ,i.e 60MHz
...
SPI_CLK_EXIT:
        ...
        SPI_CTAR_REG(SPIN[spin], 0) = (0
                                       ...
                                       //| SPI_CTAR_CPOL_MASK   //时钟极性，1表示 SCK 不活跃状态为高电平,   NRF24L01 不活跃为低电平, M8266WIFI不活跃为低电平
...
void spi_mosi(SPIn_e spin, SPIn_PCSn_e pcs, uint8 *modata, uint8 *midata, uint32 len)
{
    ...
    SPI_SR_REG(SPIN[spin]) |= SPI_SR_RFDF_MASK;                     //写1清空RFDF，标记Rx FIFO 是空的. //actually since TCF=1 as well, |= operator will clear TCF as well
    //SPI_SR_REG(SPIN[spin]) = SPI_SR_RFDF_MASK;
}
