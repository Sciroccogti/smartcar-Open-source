...
#define LCD_SDA	PTB23_OUT
#define LCD_RST PTB19_OUT 
#define LCD_DC  PTB22_OUT 
...
{
	u8 i,j,t;
	u8 img_col[120][80],img_show[60][80];
	
	for(i=0;i<120;i++)
		for(j=0;j<80;j++)
			img_col[i][j] = img[i][j*2]&&img[i][j*2+1];//行压缩，||保留1（白色）&&保留0（黑色）
	for(i=0;i<60;i++)
		for(j=0;j<80;j++)
			img_show[i][j] = img_col[i*2][j]&&img_col[i*2+1][j];//列压缩
	...
	for(i=0;i<7;i++)		//显示80*7*8像素
	{
		...
		for(j=0;j<80;j++)
		{
			...
			img_show[i*8+5][j]*32+
			img_show[i*8+6][j]*64+
			img_show[i*8+7][j]*128;	//把一列8个点压在一个字节里
