...
void LCD_clear_xy(uint16 rgb565, uint16  x,  uint16  y)     //清屏
{
    Site_t site;
    site.x=x;
    site.y=y;
 ...
 ******************************************************************************/
void LCD_line(uint16 Color)
{
    static Site_t site_new;

    for(int i=0;i<use_line;i++)
    {
	site_new.x= midline_x[i];
	site_new.y=i;
	LCD_point(site_new, Color);
        
        site_new.x= midline_x[i]-1;
	LCD_point(site_new, Color);
        
        site_new.x= midline_x[i]+1;
	LCD_point(site_new, Color);

	site_new.x= leftline_x[i]+5;
	LCD_point(site_new, RED);

	site_new.x= rightline_x[i]-5;
	LCD_point(site_new, BLUE);
        
        site_new.x= leftline_x[i]+6;
	LCD_point(site_new, RED);

	site_new.x= rightline_x[i]-6;
	LCD_point(site_new, BLUE);
    }
}
